...
describe('Async component', () => {
    ...
    test('check "currentlyReading" sections with its books', async () => {
        const shelfTitle = 'currentlyReading';
        window.fetch = jest.fn();
        window.fetch.mockResolvedValueOnce({
            json: async () => books,
        });
        render(<Bookshelf fetchedBooks={books.books} shelfTitle={shelfTitle} changeBookShelf={changeBookShelf}/>);

        const shelfTitleElement = await screen.findAllByText(shelfTitle);
        const bookTitleElement = await screen.findAllByText(books.books[0].title);

        expect(shelfTitleElement).not.toHaveLength(0);
        expect(bookTitleElement).not.toHaveLength(0);
    });
    test('check "wantToRead" sections with its books', async () => {
        const shelfTitle = 'wantToRead';
        window.fetch = jest.fn();
        window.fetch.mockResolvedValueOnce({
            json: async () => books,
        });
    ...
    test('check "read" sections with its books', async () => {
