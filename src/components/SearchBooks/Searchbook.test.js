...
describe('Async component', () => {
    ...
    test('check if search input exits', async () => {
        const inputPlaceHolder = 'Search by title or author';
        render(
            <Provider store={store}>
                <BrowserRouter>
                    <SearchBooks changeBookShelf={changeBookShelf} fetchedBooks={searchedBooks} />
                </BrowserRouter>

            </Provider>
        );

        const searchInputElement = await screen.findAllByPlaceholderText(inputPlaceHolder);
        const links = screen.getAllByRole("link");

        expect(searchInputElement).not.toHaveLength(0);
    });
    test('redirect to home page if user close search button', async () => {
        render(
            <Provider store={store}>
                <BrowserRouter>
                    <SearchBooks changeBookShelf={changeBookShelf} fetchedBooks={searchedBooks} />
